...
class State():
    ...
    _name : str                                                                                             # Name of this state
    _transitions : typing.Sequence[typing.Tuple[typing.Callable[['State', Board], bool], 'State']]          # Store the transitions of the state
    _run_thread : threading.Thread                                                                          # Hold the thread executing the action
    _status: StateStatus                                                                                    # Status of this state
    _internal_exception: Exception
    _interupted_event: threading.Event                                                                      # Event acting as a flag for interruptions
    ...
    def start(self, board: Board) -> None:
        ...
        self._run_thread = threading.Thread(target=self._execute, args=(board,))
    ...
    def wait(self, timeout: float = None) -> bool:
        ...
        """
        if self._run_thread != None and self._run_thread.isAlive():
...
class Machine(NestedState):
    ...
    def is_end(self) -> bool:
        return not self._curr_state._run_thread.isAlive() and  (self._curr_state._name == self._end_state_ids or self._curr_state._name in self._end_state_ids)
